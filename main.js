...
Emitter.prototype.reset = function () {
 ...
 var color = [~~(Math.random() * 150) + 10, ~~(Math.random() * 150) + 10,
~~(Math.random() * 150) + 10]
 this.particles = [];
...
function flipCard() {
    ...
    document.getElementById("score__result").innerHTML = "Вы справились за" + " " + Math.floor(scoreCount/2) + " шагов";
...
document.addEventListener('DOMContentLoaded', function() {
    // Находим кнопку с id="retryButton"
    let retryButton = document.getElementById('retryButton');

    // Добавляем обработчик событий для клика по кнопке
    retryButton.addEventListener('click', function(e) {
        //e.preventDefault(); // Предотвращаем стандартное действие ссылки (переход по ссылке)

        // Здесь можно добавить логику сброса игры и другие действия
        resetGame();

        // Закрываем попап завершения игры
        let endGamePopup = document.getElementById('popup__end');
        endGamePopup.style.display = 'none'; // Скрываем попап завершения игры
    });
});
